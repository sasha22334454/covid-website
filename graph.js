...
const setCountryNames = country_name => {
  document.querySelectorAll(".country-name").forEach((e) => {
    e.innerText = country_name;
  });
}
...
const populateDropdown = countries => {
  ...
  dropdown.innerHTML = country_options;
}
...
const populateGraphs = async country_data => {
  ...
  document.querySelectorAll('.chart').forEach(chart => {
    ...
    const {label, stat, large, color, bord} = chart.dataset; 
    const graph = new Chart(ctx, {
      type: 'line',
      data: {
        labels,
        datasets: [{
            label: `${label} in ${country}`,
          data: country_data.map(o => o[stat]),
          borderWidth: 2,
          fill: false,
          backgroundColor: color || '#fff',
          borderColor: bord || '#fff',
          pointRadius: 0
        }]
      },
      options: {
    ...
    if(large) {
      graph.config.options.scales.yAxes[0].display = true;
      graph.config.options.scales.xAxes[0].display = true;
      graph.config.data.datasets[0].pointRadius = 3;
      graph.config.data.datasets[0].fill = true;
      graph.update();
    }

    graph_map[chart.id] = {graph, stat, label};
